...
Shows shared `.comments/threads/*.jsonl` review threads in the VS Code editor. The Primary Side Bar provides a workspace-wide file/count overview, while VS Code's built-in **Comments** view shows the active file's expanded threads with the creation-time original text. It supports comments, replies, resolve/reopen, re-anchoring, individual comment deletion, external JSONL reload, and file relocation without MCP or agent integration.
